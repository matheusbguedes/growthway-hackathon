...
import { BookOpen, CheckSquare2, ChevronRight, Clock, Star, Target } from "lucide-react";
...
export default function PortalPage() {
  ...
  const { data: student, isLoading } = useQuery({
    queryKey: ["student", "me"],
    queryFn: () => getStudent("9054c246-b908-4eea-a494-83dc531515dc"),
  });
  ...
  if (!student) {
    return (
      <div className="min-h-screen bg-background flex items-center justify-center">
        <div className="mx-auto max-w-sm rounded-2xl border border-dashed border-border bg-muted/20 p-10 text-center">
          <div className="mx-auto mb-4 flex size-12 items-center justify-center rounded-full bg-muted">
            <BookOpen className="size-5 text-muted-foreground" />
          </div>
          <p className="text-sm font-semibold text-foreground">Acesso não liberado</p>
          <p className="mt-1.5 text-sm text-muted-foreground leading-relaxed">
            Entre em contato com seu professor para liberar o acesso ao portal.
          </p>
        </div>
      </div>
    );
        ...
        <div className="relative overflow-hidden rounded-2xl border border-border bg-card">
          ...
          <div className="relative p-6 flex items-center gap-5 flex-wrap">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-xs font-medium text-muted-foreground uppercase tracking-widest mb-0.5">Portal do Aluno</p>
            ...
            <div className="relative flex items-start justify-between gap-4">
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-baseline gap-2">
                  <span className="text-4xl font-black text-foreground tabular-nums">{stats.concluded_classes}</span>
